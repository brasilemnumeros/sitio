...
class ChartIntegration {
  ...
  setupMultiselect(config) {
    ...
    window.updateChartsFromMultiselect = (selectedIndicators, indicatorMap, forceClear = false) => {
  ...
  async updateChart(dataFiles, indicatorNames = null) {
    ...
    try {
      ...
      const jsonDataPromises = filesArray.map(file => this.chartManager.loadData(file));
      ...
      if (validData.length > 0) {
        ...
        setTimeout(() => {
          ...
          const finalNames = validData.map((jsonData, index) => {
            ...
            const indicator = this.chartManager.indicatorsConfig?.indicators?.find((ind) => ind.datafile === dataFile);
      ...
      mutations.forEach((mutation) => {
        if (mutation.attributeName === 'class') {
          if (this.chartManager.chart) {
            ...
            // Update title color
            this.chartManager.chart.options.plugins.title.color = isDark ? '#f9fafb' : '#1f2937';

            // Update axes colors
            this.chartManager.chart.options.scales.x.title.color = isDark ? '#f9fafb' : '#1f2937';
            this.chartManager.chart.options.scales.x.ticks.color = isDark ? '#d1d5db' : '#374151';
            this.chartManager.chart.options.scales.x.grid.color = isDark ? '#4b5563' : '#e5e7eb';

            this.chartManager.chart.options.scales.y.title.color = isDark ? '#f9fafb' : '#1f2937';
            this.chartManager.chart.options.scales.y.ticks.color = isDark ? '#d1d5db' : '#374151';
            this.chartManager.chart.options.scales.y.grid.color = isDark ? '#4b5563' : '#e5e7eb';

            // Update y1 axis colors if it exists
            if (this.chartManager.chart.options.scales.y1) {
              this.chartManager.chart.options.scales.y1.title.color = isDark ? '#f9fafb' : '#1f2937';
              this.chartManager.chart.options.scales.y1.ticks.color = isDark ? '#d1d5db' : '#374151';
            }

            // Update Dilma label color based on theme
            if (this.chartManager.chart.options.plugins.annotation.annotations.dilma_label) {
              this.chartManager.chart.options.plugins.annotation.annotations.dilma_label.color = isDark ? '#1f2937' : '#6b7280';
            }
